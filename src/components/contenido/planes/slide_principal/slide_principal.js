...
export default function SlidePlanes(){


	const dataSlidePlanes = async() => {
		/*=============================================
		=            CREAMOS EL DATASET               =
		=============================================*/
		
		const getSlidePlanes = await getData();
		const dataSet = [];
		

		getSlidePlanes.data.forEach((slide, index) => {
	...
	$(document).ready(function () {
		let tablaSlidePlanes = $('.table').DataTable({
			retrieve: true,
			data: dataSet,
            ...
            {title: "Imagen",
                render: function(data){
                    return `<img src=${rutaAPI}/mostrar-principal-img-planes/${data} style="width:320px">`
                }
            },
			{title: "Título"},
            { title: "Descripción" },
			...
			{title: "Acciones",
              render: function(data){
              	...
              	return `
					...
					<a href="#" class="editarInputs" data-toggle="modal" data-target="#editarSlidePlanes" data="${data}">
		...
		tablaSlidePlanes.on("order.dt search.dt", function(){


				tablaSlidePlanes.column(0, {search:"applied", order:"applied"})
				.nodes()
...
const getData = () => {
	const url = `${rutaAPI}/mostrar-principal-img-planes-data`;
	const params = {
		method: "GET",
		headers: {
