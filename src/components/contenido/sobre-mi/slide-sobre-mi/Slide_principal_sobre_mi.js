...
export default function SlideSobreMi(){


	const dataSlideSobreMi = async() => {
		/*=============================================
		=            CREAMOS EL DATASET               =
		=============================================*/
		
		const getSlideSobreMi = await getData();
		const dataSet = [];
		

		getSlideSobreMi.data.forEach((slide, index) => {
		...
		let tablaSlideSobreMi = $('.table').DataTable({
            ...
            {title: "Imagen",
                render: function(data){
                    return `<img src=${rutaAPI}/mostrar-principal-img-sobremi/${data} style="width:320px">`
                }
            },
			{title: "Título"},
            { title: "Descripción" },
			...
			{title: "Acciones",
              render: function(data){
              	...
              	return `
					...
					<a href="#" class="editarInputs" data-toggle="modal" data-target="#editarSlideSobreMi" data="${data}">
...
const getData = () => {
	const url = `${rutaAPI}/mostrar-principal-img-sobremi-data`;
	const params = {
		method: "GET",
		headers: {
