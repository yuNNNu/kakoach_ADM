...
import CrearFooter from './CrearFooter'
export default function FooterClient(){


	const dataFooterInicio = async() => {
		/*=============================================
		=            CREAMOS EL DATASET               =
		=============================================*/
		
		const getFooter = await getData();
		const dataSet = [];
	...
	$(document).ready(function () {
		let tablaFooter = $('.table').DataTable({
			retrieve: true,
			data: dataSet,
			...
			{title: "Acciones",
              render: function(data){
              	...
              	return `
					...
					<a href="#" class="editarInputs" data-toggle="modal" data-target="#editarFooter" data="${data}">
		...
		tablaFooter.on("order.dt search.dt", function(){


				tablaFooter.column(0, {search:"applied", order:"applied"})
				.nodes()
						...
						<div className="col-lg-12">
							<div className="card card-primary card-outline">
								<div className="card-header">
									<h5 className="m-0">
										<button className="btn btn-primary limpiarForm" data-toggle="modal" data-target="#crearFooter">Nuevo Footer</button>
									</h5>
								</div>
