...
export default function PlanPersonal(){
	...
	const dataPlanPersonalInicio = async() => {
		/*=============================================
		=            CREAMOS EL DATASET               =
		=============================================*/
		
		const getPlanPersonal = await getData();
		const dataSet = [];
	...
	$(document).ready(function () {
		let tablaPlanPersonal = $('.table').DataTable({
			retrieve: true,
			data: dataSet,
            ...
            {title: "Imagen",
                render: function(data){
                    return `<img src=${rutaAPI}/show-personal-plan-img/${data} style="width:320px">`
                }
            },
			{title: "Título"},
            { title: "Descripción" },
			...
			{title: "Acciones",
              render: function(data){
              	...
              	return `
					...
					<a href="#" class="editarInputs" data-toggle="modal" data-target="#editarPlanPersonal" data="${data}">
		...
		tablaPlanPersonal.on("order.dt search.dt", function(){


				tablaPlanPersonal.column(0, {search:"applied", order:"applied"})
				.nodes()
...
const getData = () => {
	const url = `${rutaAPI}/show-personal-plan`;
	const params = {
		method: "GET",
		headers: {
